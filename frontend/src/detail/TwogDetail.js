...
export default function TwogDetail() {
    const { id } = useParams();
    const twog = twogdata.twog.find((x) => x.id === id);
    const { addToCart, increase, cartItems } = useContext(CartContext);
    if(!twog) {
        return <div> Product Not Found</div>
    }
    const isInCart = (twog) => {
      return !!cartItems.find((item) => item.id === twog.id);
    };
    return (
      <div>
              ...
              <div className='col-1'>
                <div className='card card-body' >
                    <ul>
                        ...
                        <li>
                            {isInCart(twog) && (
                              <button className='button-85 block' onClick={() => {increase(twog)}}>Add More</button>
                            )}
                            {!isInCart(twog) && (<button className='button-85 block' onClick={() => addToCart(twog)}>Add to Cart</button>)}
                        </li>
                    </ul>
                </div>
              </div>
